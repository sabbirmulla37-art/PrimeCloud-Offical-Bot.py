...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    """Create a custom VPS for a user (Admin only) — 1create @user <ram_GB> <cpu_cores> <disk_GB>"""
    ...
    creating_embed = create_embed(
        "☁️ Creating VPS",
        f"Deploying ubuntu:22.04 VPS for {user.mention}\n\nPrimeCloud | VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        color=0x5865F2
    )
    ...
    try:
        ...
        # Remove the temporary creation message after successful deployment.
        try:
            await creating_message.delete()
        except (discord.NotFound, discord.Forbidden, discord.HTTPException):
            pass

        embed = create_embed("🚀 VPS Deployed!", f"{user.mention} your VPS is live! Check your **DMs** for the VPS details.", color=0x00ff88)
        ...
        embed.add_field(name="🎮 Manage",    value="Use `1manage` → **Start / Stop / Reinstall / SSH**", inline=False)
        await ctx.send(embed=embed)

        # DM user with the new clean VPS-created message.
        try:
            created_text = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            dm_embed = create_embed("✅ PrimeCloud - VPS Created!", "Your VPS has been successfully deployed by an admin!", color=0x00ff88)
            dm_embed.add_field(name="**VPS ID:**", value=f"`#{vps_count}`", inline=False)
            dm_embed.add_field(name="**Container Name:**", value=f"`{container_name}`", inline=False)
            dm_embed.add_field(name="**Configuration:**", value=f"`{ram}GB RAM • {cpu} CPU Cores • {disk}GB Disk`", inline=False)
            dm_embed.add_field(name="**Status:**", value="`RUNNING`", inline=False)
            dm_embed.add_field(name="**OS:**", value="`ubuntu:22.04`", inline=False)
            dm_embed.add_field(name="**Created:**", value=f"`{created_text}`", inline=False)
            dm_embed.add_field(
                name="⌯⌲ Management",
                value="• Use `1manage` to start/stop/reinstall your VPS\n• Use `1manage` → SSH for terminal access\n• Contact admin for upgrades or issues",
                inline=False
            )
            dm_embed.add_field(
                name="⌯⌲ Important Notes",
                value="• Full root access via SSH\n• Docker-ready with nesting and privileged mode\n• Backups are not provided",
                inline=False
            )
            dm_embed.set_footer(text="Powered by PrimeCloud • Premium VPS Management")
        ...
        try:
            await creating_message.edit(embed=create_error_embed("Creation Failed", f"Error: {str(e)}"))
        except Exception:
    ...
    try:
        ...
        try:
            ...
            dm_embed.add_field(
                ...
                value="Use `1manage` in the server to **Start / Stop / Reinstall / SSH**",
                inline=False
            )
...
@bot.command(name='coustem_command')
@is_admin()
async def coustem_command(ctx, command: str, *, response: str):
    """Create/update a custom command. Usage: 1coustem_command <command> <response>"""
    command = command.strip().lower().lstrip('1').strip()
    if not command or command in bot.all_commands or command == 'coustem_command':
        await ctx.send(embed=create_error_embed(
            "Invalid Command",
            "Choose a custom command name that is not already used by the bot."
        ))
        return
    response = response.strip()
    if not response:
        await ctx.send(embed=create_error_embed("Invalid Response", "Response cannot be empty."))
        return
...
def build_help_pages(is_user_admin, is_user_main_admin):
    ...
    credits_embed.add_field(name="💳 Commands", value=(
        ...
        "`1buyc` — Get payment info (UPI/PayPal/Crypto)\n"
    ...
    if is_user_admin:
        ...
        admin_embed.add_field(name="🖥️ VPS Control", value=(
            "`1create @user <ram_GB> <cpu_cores> <disk_GB>` — Create custom Docker VPS\n"
        ...
        admin_embed.add_field(name="📊 Info & Economy", value=(
            ...
            "`1coustem_command <command> <response>` — Add/update a custom command"
        ), inline=False)
...
async def on_message(message):
    ...
    # Handle saved custom commands using the new numeric prefix (e.g. 1hello).
    if message.content.startswith('1') and not message.content.startswith('1 '):
        custom_name = message.content[1:].split(maxsplit=1)[0].strip().lower() if message.content[1:].strip() else ''
        if custom_name in custom_commands and custom_name not in bot.all_commands:
            await message.channel.send(custom_commands[custom_name])
            return
...
if __name__ == "__main__":
    ...
    if not token:
        raise SystemExit("DISCORD_BOT_TOKEN is not set. Set it before starting the bot.")
    bot.run(token)